...
- canonical 액션 카드는 하단 단일행에서 `quote-change`를 왼쪽 끝, `quote-rate`를 오른쪽 끝에 배치하며, 두 값의 합산 폭이 안전 영역을 넘을 때만 단계적으로 축소해 가운데 간격을 보장
- 방향 화살표와 `+`/`-` 부호는 `quote-change`에만 표시하고 `quote-rate`는 절댓값 퍼센트만 표시
- 국내·미국 시세 방향 색상은 한국 사용자 관례에 맞춰 상승=빨강, 하락=파랑, 보합=회색으로 표시하고 연결·오류·회복 상태 색상과 별도 상수로 관리
