...
export class DomesticStockAction extends SingletonAction<DomesticStockSettings> {
  ...
  override async onWillAppear(
    ...
    if (updateMode === "poll") {
      const staleThreshold = Math.max(pollIntervalSec * 2 * 1000, DOMESTIC_STALE_AFTER_MS);
  ...
  private isStale(actionId: string): boolean {
    ...
    const threshold = this.staleAfterByAction.get(actionId) ?? DOMESTIC_STALE_AFTER_MS;
