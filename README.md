...
`연결 테스트` 버튼은 저장과 별개로 동작합니다.

- 현재 PI에 입력된 `App Key` / `App Secret`을 우선 사용합니다.
- 두 필드가 모두 비어 있을 때만 이미 저장된 Global Settings 자격증명으로 fallback 합니다.
- 둘 중 하나만 비어 있으면 저장된 값과 섞어 쓰지 않고, 자격증명이 없는 것으로 처리합니다.
- 연결 테스트는 설정을 저장하지 않습니다.
- 현재 버튼 섹션에 유효한 종목 입력이 있으면 자격증명 확인 뒤 해당 종목의 REST 현재가도 함께 검증합니다.
- 현재 버튼 입력 형식이 잘못된 경우 네트워크 요청 전에 버튼 설정 오류를 먼저 표시합니다.
- 테스트 중 버튼 라벨은 `연결 확인 중...`으로 바뀌고 중복 클릭이 막힙니다.
- 검증 범위
  - `App Key` / `App Secret` 유효성 확인
  - KIS API 연결 가능 여부 확인
  - 현재 버튼의 `종목코드` 또는 `티커/거래소`가 REST 현재가 기준으로 유효한지 확인
  - 실제 WebSocket 구독 성공 여부까지는 확인하지 않음
- 결과 메시지
  - 성공: `KIS API 자격증명을 확인했습니다.`
  - 자격증명 + 종목 확인 성공: `KIS API 자격증명과 현재 버튼 종목 설정을 확인했습니다.`
  - 자격증명 없음: `App Key와 App Secret을 입력한 뒤 다시 시도하세요.`
  - 인증 실패: `App Key 또는 App Secret을 확인하세요.`
  - 국내 종목 오류: `종목코드를 확인하세요.`
  - 미국 종목 오류: `티커와 거래소를 확인하세요.`
  - 네트워크/기타 실패: `KIS API 연결을 확인하지 못했습니다. 잠시 후 다시 시도하세요.`
...
│   │   └── __tests__/
│   ├── property-inspector/
│   │   ├── connection-test.ts        # PI 연결 테스트 처리
│   │   └── message-contract.ts       # PI ↔ Plugin 메시지 타입
│   ├── renderer/
