...
- 각 주식 버튼의 PI에서 **고급 설정 → 화면 반영 방식**을 열어 `실시간 (50ms 최신값 병합)` 또는 `스로틀링`을 선택합니다. 스로틀 간격은 500~1000ms 범위에서 100ms 단위로 입력하고 **고급 설정 저장**을 눌러야 하며, 국내·미국 주식 버튼 모두에 전역 적용됩니다. 실시간 모드에서는 입력이 비활성화되지만 저장값은 보존됩니다.
...
5. 필요하면 고급 설정에서 데이터·화면 반영·백업 정책을 조정하고 **고급 설정 저장**을 누릅니다.
