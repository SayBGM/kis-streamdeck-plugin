...
- **렌더링 성능 최적화** (v1.2.0)
  ...
  - LRU 캐시 크기 200 → 500개
- **에러 상태 표시 강화** (v1.3.0)
  - 인증 실패, 네트워크 오류, 유효하지 않은 종목코드를 버튼에 직접 표시
  - 자격증명 미설정, 인증 실패, 연결 오류, 종목 오류를 아이콘으로 구분
- **데이터 지연 표시** (v1.3.0)
  - WebSocket 데이터가 20초 이상 미수신 시 버튼에 "지연" 텍스트 표시
- **연결 회복 알림** (v1.3.0)
  - 연결 장애 복구 시 2초간 회복 성공 알림 표시
- **Property Inspector 개선** (v1.3.0)
  - 종목코드/티커 실시간 형식 검증 (국내: 6자리 숫자, 해외: 1~6자 영문)
  - "연결 테스트" 버튼으로 API 자격증명 즉시 검증
- 장 상태 표시
  - `PRE` / `REG` / `AFT` / `CLOSED`
